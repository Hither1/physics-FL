...
#with h5py.File("../raw-data/sampled/sir_valid_rk.hdf5", "w") as f:
S, I, R = [], [], []
mu, sigma = 0, 0.001
for i in range(2048):
    ...
    def Model(days, k, b): # agegroups,
        ...
        S_i, I_i, R_i = ret.T
        S.extend(S_i + np.random.normal(mu, sigma, 257))
        I.extend(I_i + np.random.normal(mu, sigma, 257))
        R.extend(R_i + np.random.normal(mu, sigma, 257))
    ...
    t, S, I, R = Model(257, k, b) # 2048, 256,

df = pd.DataFrame(np.array([S, I, R]).T)
df.to_csv("../SIR_val_noisy_x.csv", index=False)
